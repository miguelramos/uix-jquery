...
;(function($, undefined){
    ...
    _create: function() {
    	...
    	this.hiddenSelect = $('input').tag({'name':'icon[]','type':'hidden','class':'id-icon-select'});
    	...
    	this.element.on('iconsselect', self._onSelect);

    	this.element.append(this._setupIcons(map));
    ...
    _setupIcons: function(map){
    	...
    	$.each(map, function(index, icon){
    		icon.on('click', function(evt){
    			...
    			if($(this).parent().find('.icon-select')){
    				$(this).parent().find('.icon-select').removeClass('icon-select');
    				$(this).parent().find('.ui-icon-check').css('display','none');
    			}

    			$(this).children('.ui-icon-check').css('display','block');
    			$(this).addClass('icon-select');

    			self._trigger('select', evt, {'icon': $(self.options.tag, icon).attr('class')});
    		});
    	...
    	container.append(this.hiddenSelect);

    	return container;
    },
    _onSelect: function(evt, args){
    	var target = $(evt.originalEvent.currentTarget);
